...
**Why is Auth.js split into `auth.config.ts` and `auth.ts`?** Even on the Node.js runtime, `proxy.ts` is compiled as its own bundle, separate from route handlers and Server Components — and `serverExternalPackages` in `next.config.ts` (which lets `sequelize`/`pg` skip bundling) only applies to that main bundle, not the proxy one. `auth.config.ts` is a provider-less base config safe for `proxy.ts` to import (it also sets `trustHost: true` — Auth.js only auto-trusts the request Host header on Vercel; every other environment, including `next start` locally, needs this set explicitly or sign-in redirects to `/api/auth/error` with `UntrustedHost`). `auth.ts` adds the real Credentials provider (which needs `bcryptjs` + Sequelize) on top of it for route handlers/Server Components. Importing `@/lib/auth/auth` from `proxy.ts` drags Sequelize into the proxy bundle and breaks it at runtime with `Error: Please install pg package manually`.

**Why does `src/lib/db/sequelize.ts` import `pg` directly and pass `dialectModule: pg`?** Sequelize loads its Postgres driver internally via `require(moduleName)`, where `moduleName` is a variable — no bundler or file-tracer (Turbopack, Next's own output tracing, Vercel's function packaging) can see that as a real static dependency on `pg`. It can work locally (`next dev`, `next build`, even `next start`) purely by luck of how each tool's trace heuristics happen to behave, and still get excluded from an actual deployed Vercel function, failing at runtime with `Error: Please install pg package manually`. A static `import pg from 'pg'` plus `dialectModule: pg` sidesteps the dynamic `require` entirely — Sequelize uses the module object directly instead of trying to resolve it.
...
There's no automated test suite yet. The working pattern used throughout this project's history: run `npx tsc --noEmit`, `npm run lint`, and `npm run build` after every change, then actually drive the feature in a real browser (a Playwright script) rather than trusting that a clean build means the feature works — several real bugs (a stale Sequelize association wiring issue, a Dexie/Tiptap focus race) were only caught this way, not by the type checker.

**`npm run dev` is not enough for anything touching Sequelize, the proxy, or Auth.js config.** Two production-only bugs (the proxy bundle pulling in Sequelize, and Sequelize's dynamic `require('pg')` getting missed by bundler tracing) never appeared in `next dev` or even a plain `next build` — both only surfaced by actually running `npm run build && npm run start` and exercising a real request that touches the database. Before considering a change to `next.config.ts`, `src/proxy.ts`, `src/lib/auth/**`, or `src/lib/db/sequelize.ts` done, verify it against `next start`, not just the dev server.
